...
function waitForElementToSet(chkfunc, callback, checkFrequencyInMs, timeoutInMs) {
...
window.onload = function() {
  ...
  function chk(){
    const v=document.querySelector("#video-play-video1 > div.vc-vplay-container.non-selectable > video");
    if(v!=null){
      return !(v.src.includes("intro.mp4")||v.src.includes("preloader.mp4"))
    }
    return false;
  }

  waitForElementToSet(chk,()=>{
    const video = document.querySelector("#video-play-video1 > div.vc-vplay-container.non-selectable > video");
    if(!video||video.src.includes("intro.mp4")||video.src==""){console.log("not found");return};
    ...
    const workerUri = chrome.runtime.getURL('needles-worker.js')// GM_getResourceUrl("needles-worker.js");
  
    const mode = 'integrated'; /// momentary | short-term | integrated
    source.connect(gainNode).connect(gainNode2).connect(audioContext.destination);
  
    // add new volume slider
    //volumebar.appendChild()
    // volumebar.removeChild(volumebar.firstChild);
    ...
    chrome.storage.local.get(["vol"],(items)=>{
      ...
      volumeinput.addEventListener("change",function(ev){
        chrome.storage.local.set({vol: this.value});
      })
    ...
    var preLoudnessMeter = new LoudnessMeter({
        source: source,
        modes: ["integrated","momentary"],
        workerUri
    });
  
    var postLoudnessMeter = new LoudnessMeter({
        source: gainNode,
        modes: ["short-term"],
        workerUri
    });
    ...
    var aimlufs=-13;
    var minsanitycheck=-60;
    var maxgainrate=10;
    var pg=100,ps=100;
    var skiptwo=0;
    
    function coeffexpGain(x){
        //[9.14239548e-01 1.28404479e+00 1.44288357e-04] a**(b*x)+c
        return 0.89590812**(1.04749473*x);
    }
  
    preLoudnessMeter.on('dataavailable', function (event) {
        if(skiptwo<3){
          skiptwo+=1;
          return;
        } 
        pg=event.data.value;
        if(event.data.mode=="momentary" && (gainNode.gain.value-coeffexpGain(pg-aimlufs))>0.1){
          gainNode.gain.value=coeffexpGain(pg-aimlufs);
        }else if(event.data.mode=="integrated"){
          let aimval=1.0;
          if(pg>minsanitycheck){
              aimval=coeffexpGain(pg-aimlufs)
          }
          if(aimval>maxgainrate){
              aimval=maxgainrate;
          }
          gainNode.gain.value+=(aimval-gainNode.gain.value)/4;//for smooth transition
        }
    })
  
    postLoudnessMeter.on('dataavailable', function (event) {
        ps=event.data.value;
        if(pg!=100){
            console.log(pg,ps,pg-ps,gainNode.gain.value);
            pg=100;ps=100;
        }
    })
  
    preLoudnessMeter.start()
    postLoudnessMeter.start()  
    audioContext.resume();
  },1000,100000); 
}
